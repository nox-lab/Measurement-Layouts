...
def setupModel(taskResults, covar_matrix):
    m = pm.Model()
    ...
    with m:
      ...
      ability_visual = pm.MvNormal("ability_visual", mu = np.zeros(T), chol=covar_matrix*sigma_vis, shape=T)
      
      ability_nav = pm.MvNormal("ability_navigation", mu = np.zeros(T), chol=covar_matrix*sigma_nav, shape=T)
      if (includeIrrelevantFeatures) :
        ability_bias_rl = pm.MvNormal("ability_bias_rl", mu = np.zeros(T), chol=covar_matrix*sigma_bias, shape=T)                  # [-inf,inf] A parameter to determine whether left or right have an influence. It's expected to be zero, but negative would be left influence and positive a right influence (or vice versa :-)
...
if __name__ == "__main__":
  ...
  covar_matrix = rbf_kernel(time_steps, length_scale = 1)
...
# %%
  m = setupModel(successes, covar_matrix=covar_matrix)
  ...
  for cap, true_mus, fig, ax in [("ability_bias_rl", capability_bias, bias_fig, bias_ax), ("ability_visual", capability_vis, vis_fig, vis_ax), ("ability_navigation", capability_nav, nav_fig, nav_ax)]:
      ...
      plt.xlabel("timestep")
      plt.legend()
      fig.savefig(f"estimated_{cap}_pc.png")
