...
def setupModel(taskResults, cholesky):
    ...
    with m:
      ...
      ability_nav = scaledBeta("ability_visual", 1, 1, abilityMin["visualAbility"], abilityMax["visualAbility"], T)
      ability_visual = scaledBeta("ability_navigation", 1,1, abilityMin["navigationAbility"], abilityMax["navigationAbility"], T)

      if (includeIrrelevantFeatures) :
        ability_bias_rl = pm.Normal("ability_bias_rl", mu = 0, sigma = 1, shape = T)                  # [-inf,inf] A parameter to determine whether left or right have an influence. It's expected to be zero, but negative would be left influence and positive a right influence (or vice versa :-)
...
capability_bias = logistic((time_steps- 30)/10)
